...
switch($_GET["action"]) {
	case "add":
		if(!empty($_POST["quantity"])) {
			$productByCode = $db_handle->runQuery("SELECT * FROM continental WHERE code='" . $_GET["code"] . "'");
...
<a class="btnpg" href="index6.php?action=empty">Empty Cart</a>
...
<?php		
    foreach ($_SESSION["cart_item"] as $item){
		...
		?>
				...
				<td style="text-align:center;"><a href="index6.php?action=remove&code=<?php echo $item["code"]; ?>" class="btnRemoveAction"><img src="images/icon-delete.png" alt="Remove Item" /></a></td>
				</tr>
				<?php
...
<div id="product-grid">
	...
	<?php
	$product_array = $db_handle->runQuery("SELECT * FROM continental ORDER BY id ASC");
	if (!empty($product_array)) { 
		foreach($product_array as $key=>$value){
	?>
		<div class="product-item">
			<form method="post" action="index6.php?action=add&code=<?php echo $product_array[$key]["code"]; ?>">
