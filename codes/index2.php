...
switch($_GET["action"]) {
	case "add":
		if(!empty($_POST["quantity"])) {
			$productByCode = $db_handle->runQuery("SELECT * FROM chinese WHERE code='" . $_GET["code"] . "'");
...
<a class="btnpg" href="index2.php?action=empty">Empty Cart</a>
...
<div id="product-grid">
	...
	<?php
	$product_array = $db_handle->runQuery("SELECT * FROM chinese ORDER BY id ASC");
